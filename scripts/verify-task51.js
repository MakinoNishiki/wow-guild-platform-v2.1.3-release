...
//      B1 全量计数「共 2062 件」+ 首页 60 卡 + 分页「第 1/35 页」+ 首页占位图数=库内计算值；
...
const cnt = {
  total: () => dbRows.length,
  firstPagePlaceholder: () => ordered().slice(0, 60).filter(r => r.icon_file_id == null).length,
...
(async () => {
  ...
  try {
    ...
    check('B1b 首页卡片数=60（分页 60/页定案）', b1.cards === 60, `卡=${b1.cards}`);
    check('B1c 分页信息「第 1/35 页 · 共 2062 件」', b1.pager.includes('第 1/35 页') && b1.pager.includes('共 2062 件'), b1.pager);
    check(`B1d 首页占位图数=库内新序首 60 件缺图标数（${cnt.firstPagePlaceholder()}，#51-补丁 第一节排序口径：房间沉底后装饰件图标齐备）`, b1.placeholder === cnt.firstPagePlaceholder(),
      `页面=${b1.placeholder} 库=${cnt.firstPagePlaceholder()}`);
    // B1e（#51-补丁 第一节）：首页 60 卡零房间卡（房间沉底）+ P1 首卡=装饰序第 1 件（RPC 实测 decorOnly[0]）
    ...
    check('B1e 排序沉底：首页 60 卡零房间卡 + 首卡=装饰序第 1 件',
      b1e.length === 60 && b1e.every(rid => !roomRids.has(rid)) && b1e[0] === decorOnlyRows[0].record_id,
    ...
    // B3 翻页主链路【2026-09-18 排序口径变更：装饰在前房间沉底（任务书 #51-补丁 第一节）——
    //    旧口径=全量 record_id asc 第 61 件（487）；新口径=装饰序第 61 件（RPC 实测 decorOnly[60]）】
    ...
    check('B3 翻页：第 2 页首卡=装饰序第 61 件 record_id（新口径）+ 分页文本', b3.first === decorOnly[60].record_id && b3.pager.includes('第 2/35 页'),
      `首卡=${b3.first} 期望=${decorOnly[60].record_id}（P1 首卡=${firstCardP1}）${b3.pager}`);
    ...
    // ==================== #51-补丁 第三节：卡片信息扩充断言 ====================
    // B8a 首页每张卡含分类行 .dh-cat 与来源行 .dh-src
    ...
    check('B8a 卡片 DOM：60/60 卡含分类行 .dh-cat + 来源行 .dh-src', b8a.n === 60 && b8a.ok, `卡=${b8a.n} 齐=${b8a.ok}`);
    ...
    await page.evaluate(() => { document.getElementById('dhSearchClear').click(); });
    await sleep(300);
    // B8c 来源未知件：来源「无来源」筛选下，首屏「来源未知」卡数=库内同语义（sources 空且 source_text 空）
    await clickChip('dhSrcChips', '无来源');
    const expUnknown = ordered().filter(r => (!Array.isArray(r.sources) || !r.sources.length))
      .slice(0, 60).filter(r => !r.source_text || !r.source_text.trim()).length;
    ...
    check(`B8c 来源未知件渲染「来源未知」灰字（首屏 ${expUnknown} 件=库内同语义）`, b8c === expUnknown, `页面=${b8c} 库=${expUnknown}`);
    await clickChip('dhSrcChips', '全部');
    // B8d 房间件分类行=「房间」
    await clickChip('dhFeatChips', '房间/户型');
