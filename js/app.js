...
const changelogData = [
  ...
  },
  {
    ...
    type: 'feature',
    typeLabel: '新增功能',
    title: '首页双入口大卡 + 登录墙公示链接 + 无公会遮罩主次调换（任务书 #52 WP2 / REQ-137 一期收尾 / REQ-138）',
    summary: '①仪表盘顶部新增双入口大卡（统计卡之前，全角色可见）：左「公会团队管理」（考勤、装备、心愿单——团长每晚的行政工作，十分钟打完）点击跳考勤页，右「家宅」（2062 件家宅装饰全量目录，来源筛选一键到位）点击跳家宅图鉴页签；复用 .card 体系，hover 提一档亮、按下缩放反馈，窄屏纵排。②未登录登录墙底部新增公示链接行「无需登录：家宅图鉴公示 · 副本掉落公示」（新 Tab 打开，样式降一级不抢登录主流程）。③无公会遮罩主次调换（REQ-138）：邀请码加入提为主路径（btn-primary + 引导「团员请向会长索取 8 位邀请码」），创建公会降次级（btn-ghost + 「你是会长？创建新公会」），顶部提示语改「加入公会后即可使用全部功能」；handleCreateGuild/handleJoinGuild 零逻辑变更。',
    details: [
      '纯前端：index.html 结构 + main.css 样式（.entry-cards/.auth-public-links/.auth-field-hint），零数据链路改动',
      '遮罩调换动机：多数注册者是团员，旧布局创建公会居视觉主位易引导误建会（同名山寨会=数据孤岛）',
      '遮罩文案零 emoji（上传通道破坏在案）；公示链接新 Tab 打开保持登录页驻留'
    ]
  },
  {
    id: 'v3.2.0-task52-nav-groups',
    version: 'v3.2.0',
    date: '2026-09-19',
    type: 'refactor',
    typeLabel: '模块调整',
    title: '侧边栏导航两组式 + 小宠物图鉴预留位（任务书 #52 WP1 / REQ-137 一期收尾）',
    summary: '侧边栏导航分两组渲染：「公会团队管理」（仪表盘/成员/考勤/装备/心愿/报表/数据管理/数据中心/副本掉落，组内维持原顺序）与「家宅」（家宅图鉴 + 小宠物图鉴预留位——灰态禁用、不可点、title「数据侦察中，敬请期待」，REQ-137 二期预留）；更新日志移出分组沉底至用户中心之上（站点信息项非团队管理功能）。组头为纯视觉小标签（11px/弱色/字距微放宽），不可点不参与路由；全部 data-page/onclick/角色显隐逻辑零变更。顺手修订：侧边栏品牌标题字重 600→700（对齐 UI 设计规范 v2 §3「侧边栏标题 16/700 金」）。',
    details: [
      '拖拽排序（REQ-105）最小适配：组头/预留位不带 .nav-item 类、不入 nav_order 偏好、draggable 永 false；重排/落定后组头自动归位至组内首项之前，刷新后自定义顺序保持、换账号序不串（BUG-078 快照链）',
      '≤768px 侧栏整体隐藏（底部 Tab 导航），组头窄屏不可见属移动封存已知取舍',
      '数据中心超管显隐、通知红点宿主、问题反馈入口等既有行为全部不变（verify-task52-wp1.js 43/43）'
    ]
  },
  {
  ...
  },
  {
    ...
    type: 'fix',
    typeLabel: '问题修复',
    title: '掉落数据残项订正：塞塔里斯 7 件数值+2 件非大米池删除+珍玩装等订正（sql/29）',
    summary: '纯数据订正（schema 不动）：①删除 2 件非大米池掉落——防咬手套（塞塔里斯神庙/米利克萨，普通难度专属）与黎明之刃的战刃（密谋小径/利希尔·烬怒，外观解锁件）；②塞塔里斯神庙/塞塔里斯的化身 7 件订正为 292 勇士 1/6 档实测数值（official_item_id 置换 159xxx→239xxx，主/副属性键名数组与数值同步，icon_id/ilvl 不动）；③蛰伏盘蛇珍玩（烈毒之渊/乌拉特克）物品等级 344→324——游戏内 tooltip 截图实证，兑换物以冒险手册自有档轨（285/298/311/324）为准。',
    details: [
      '影响行数：DELETE 1×2 + UPDATE 1×8 精确吻合；S2 大米 207→205 件，公示页当前赛季 320→318 件',
      '同误标自查：slot=套装兑换物 且 ilvl=344 全库仅此 1 件；盘卷祭坛无兑换物掉落，不误伤',
      'BUG-101 跳过清单 8 件销账（7 件订正 + 防咬手套确证普通难度专属删除）；备份落 backup/2026-08-16-sql29/（四表全量+10 行全行 JSON 可回插）'
    ]
  },
  {
  ...
  },
  {
    ...
    summary: '插件 1.0.26 定版——档扫终版（按实例自适应有效档探测，世界首领类无难度维单档采集）+BUG-097 枚举回滚（多 BOSS 副本 2 号位起全 0 根治）+BUG-098 重试重选（冒烟后首跑劣化根治）+BUG-095 完整性质量门加件数量级闸+BUG-099 毒咒畸形色码加固剥码（hex 定案：色值段内嵌空格，四版全灭同一根因）。真机全绿：全量 A/B 双等 381 件（run 163904/164239）、毒咒八件 8/8、大米饰品 effect 27/27。',
    details: [
      '冒烟五连（1.0.22~1.0.26）+全量 A/B 双绿，判绿五条+附加项全过（细则顾问侧台账在案）',
      'BUG-100 立案（P2）：伪实例空掉落占位误伤 095 基线重建——修法定向=095 质量门占位检查排除已被 BUG-087 过滤的伪实例（或过滤前置），随定版后首版小修',
      '观察项：effect 数值缩放抖动（258518/159618 两件 A/B 微差）——S2 录库口径=effect 存史诗档文本、数值非稳定键'
    ]
  },
  {
    id: 'v3.2.0-addon-1015-s2-fix',
    version: 'v3.2.0',
    date: '2026-08-13',
    type: 'fix',
    typeLabel: '问题修复',
    title: '插件 1.0.15：S2 掉落导出链根治（BUG-083~090 两跑定损+D 组全量）',
    summary: '插件 1.0.15：S2 掉落导出链根治——修复连跑爆栈与跨难度错位、逐段提交中断不丢数据、链中断看门狗红字提醒、伪实例过滤与导出日志可观测；写链路 syncActivity 收口统一入口。',
    details: [
      '待运营真机一次验收会话：跑 A→同会话不 reload 跑 B→/reload（可选跑 C 强制中断验断链留存），V1-V13 全判（步骤卡 Q4；含 271876 effect 全文+毒咒、后二必扫到档、双跑计数一致、跨跑带档率稳定）',
