...
1. 游戏内装载插件 1.0.6 → `/wjdc all` → `/reload` → 把 SavedVariables 文件发我（运营测试步骤卡第 6 步已含数值字段预期）。
   - **进度留痕（2026-08-07）**：1.0.6 首轮全量实跑零 lua 报错、两段完整跑完（gsub 修复生效）；但物品缓存冷启动致 failed 量过大（约采 79 件/预期约 411 件，风行者之塔/魔导师平台/执政团之座/通天峰 0 件），文件不可用于回填。已指引运营 /reload 重跑 2–4 轮至红字基本消失后再发文件；
2. 我转换后交回 load JSON，由服务通道覆盖入库（数值列写入）；
3. sql/19 迁移已执行（2026-08-07，见《TASK-028-WP1-sql19迁移执行报告》）——回填后 REST 复核：当前赛季装备 `secondary_values` 非空率 ≥95%（玩具/杂项类可空），抽查 5 件与游戏内数值一致（运营截图对照）；
4. 密钥运营私聊给，不落地仓库。
