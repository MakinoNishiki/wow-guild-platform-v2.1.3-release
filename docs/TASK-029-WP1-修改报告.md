...
> 状态：**已交付待验收**（sql/20 已上生产、probe 定音 link 通道证伪回退生效、格式冻结 v2 在案）
...
5. **真机定音（2026-08-08，运营 1.0.7 `/wjdc probe 1` 实测，截图运营存档）**：EJ 切档 API 存活——试切 LFR(17) 读回正常、还原 Normal(14) 正常；**但 link 通道不实：切档后下游无可读的档位缩放**（四档缩放实证段未获得可读的档位差异数值），与任务书 #26-fix4 稀疏表真机笔记同向——wiki 结构体的 `link` 字段在 12.x 实机不可用于数值采集。**1.0.7 运行时回退按设计生效**：通道判死后自动回退单档采集（tiers 不产出），单档导出与转换/回填链全功能完好，零数据污染。
...
1. ~~sql/20 迁移执行~~ ✅ 已完成（2026-08-08，见《TASK-029-WP1-sql20迁移执行报告》：四列在场、行数 190/221 零漂移、REST anon 读 200）；
2. ~~覆盖插件 1.0.7 → `/wjdc probe 1` 定音~~ ✅ 已完成（2026-08-08：**link 通道证伪，回退生效**，见一节第 5 条）；
3. 后续由运营拍板：四档数值来源需新通道（见六节第 1 条迭代方向），冻结 v2 格式与 tiers 两列已就绪——新通道一旦落地，1.0.7 运行时探测 + 转换器透传 + 回填脚本原链路直接复用，零返工。

## 六、遗留与说明

1. **四档数值来源真机证伪（2026-08-08 probe 定音）**：EJ 切档 API 存活但 12.x 下游无可读缩放（link 通道不实），1.0.7 回退单档生效、tiers 不产出——属任务书预留回退路径，转换器/回填/口径/冻结 v2 全部不浪费。**迭代方向存档**（另立小案，运营拍板后施工）：① `C_Item.GetItemStats(itemLink)` + ItemBonus.db2（wow.tools）缩放 bonus/装等 delta 数据外购，自维护逐版本漂移表；② 顾问侧文章/OCR 管道（套装效果同款）按档采数值；③ 放弃四档、WP2 卡面按单档口径落（任务书已声明 WP3 不受影响）。
2. `_CMP_FIELDS`（对账差异字段集）本次未加 tiers 列——S2 翻牌对账口径不变（同 WP1 对 values 的处置）。
3. 站点版本串不动（任务书红线：站点版本随下次前端改动再递增）；更新日志因同属站点版本门禁未补录，随下次前端版本统一补（REQ-081 同款先例）；WP2 卡面难度选择器档期在 #28 收尾后另排。
4. S1 回填脚本（`backup/2026-08-08-task29-wp1/backfill_tiers.py`）备妥未执行——四档通道证伪后暂无 tiers 数据可填，留待新通道落地复用。
