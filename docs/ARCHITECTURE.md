...
├── package.json                # 项目配置
├── test/
│   └── server-security.test.js # HTTP 与 service_role 代理安全边界回归测试
├── PRD.md                      # 产品需求文档
├── DESIGN.md                   # 设计规范
├── AGENTS.md                   # 项目说明
...
静态服务采用白名单：只公开 `index.html`、`assets/`、`css/` 和 `js/`。`.env`、服务端源码、SQL、脚本、任务书与内部文档默认返回 404。

HTTP 边界同时限制为：默认仅监听 `127.0.0.1`、API 请求体最大 1 MiB、写请求只接受 JSON、CORS 默认仅允许 `https://ddctl.com`，并统一设置基础安全响应头。

service_role 代理采用失败关闭：未知表/RPC、空写入、无精确过滤的修改或删除、无法确认公会归属的操作一律拒绝；公会创建者与通知 RPC 用户必须等于当前 JWT 用户。
