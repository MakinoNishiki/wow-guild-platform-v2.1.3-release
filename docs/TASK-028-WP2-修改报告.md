...
## 五、基线口径勘误（验收复核后三标勘误，2026-08-08 定稿）
原版归因「数据中心正常录入变动」不实，验收指令改写的「杂项标记多圈 2 行」经取证亦不成立。真实归因：**施工时 343 系本报告口径算错**——用 boss_loot 全表 190−51 得团本 139，未按默认视图（当前赛季 S1）过滤；S1 团本实为 188−51=137，正确 S1 默认全集 = 137+204 = **341**。

取证链（2026-08-08，anon REST 直查 + 真浏览器实测）：
1. 内克扎莉的兜帽（头部/皮甲）、盘魂玺戒（手指/戒指）两行 slot/item_type 标注正确，**不在** 68 行杂项（boss_loot 51 + dungeon_loot 17）之内；杂项 68 行逐行核对无此两件。
2. 两件归属链：boss「盘魂者内克扎莉」→ 团本「烈毒之渊」→ 赛季 **S2**（is_current=false）；页面默认赛季 = S1，故默认视图不渲染——这是赛季过滤的正常行为，非杂项规则误伤。两行 created_at=updated_at=2026-07-29，验收周期内零数据变动（boss_loot 190 = S1 188 + S2 2，verify 脚本报的 188 为赛季过滤后口径）。
3. 真浏览器实测（chromium，/data.html）：S1 默认全集 341；切 S2 后两件均正常渲染（S2 全集=2，搜「兜帽」「玺戒」各命中 1）；68 行杂项在两赛季视图均零渲染。

结论：杂项排除规则（slot='杂项' 装载层过滤）无误圈、无误伤，68 行恰为 68 行真杂项；两件 S2 装备在 S2 视图全程可见，无需代码修复。三个 verify 脚本期望值均为 REST 实抓动态计算（过滤 slot=杂项），不受口径影响；任务书 341 旧口径按裁定⑤作废。
