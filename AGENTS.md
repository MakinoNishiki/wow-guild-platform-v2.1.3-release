...
├── data.html           # 副本掉落重定向壳（任务书 #59 WP3：JS 跳 index.html#/team/lootdrop + meta 兜底，保留 track.js 旧口径 PV，不删文件）
├── decor.html          # 家宅图鉴重定向壳（任务书 #59 WP3：JS 跳 index.html#/house/decor + meta 兜底，保留 track.js 旧口径 PV，不删文件）
├── css/
│   ├── main.css        # 全局样式（WoW 暗色主题 + 认证界面样式）
│   ├── data-public.css # 副本掉落双壳共用（.dp-* 前缀 + #page-lootdrop 作用域，零泄漏）
│   └── decor-public.css# 家宅图鉴双壳共用（任务书 #51，.dh-* 前缀，零泄漏）
├── js/
│   ├── cloud.js        # 云端数据层（Supabase 集成：认证、公会、数据同步）
│   ├── dataPublic.js   # 副本掉落渲染层（双壳单一真源 window.DPLootDrop；公开壳自动挂载）
│   ├── decorDict.js    # 家宅图鉴词表单一真源（任务书 #51：价格/分类/尺寸/来源词表，顾问终审落常量）
│   ├── decorData.js    # 家宅图鉴数据层+渲染层（双壳单一真源 window.DecorCatalog；decor.html 自任务书 #59 WP3 起重定向壳化、保留引用但惰性不挂载）
...
12. **副本掉落**（page:lootdrop + data.html） - REQ-086 收官（任务书 #28 WP5，原「数据公示」更名）：**任务书 #59 WP3 起公开壳 data.html 改为重定向壳（JS location.replace 跳 index.html#/team/lootdrop，query 透传 + meta refresh 兜底；body 摘 data-public-body 类故渲染层惰性零请求；track.js 保留，PV page='data' 旧口径不断）**；双壳同一渲染层 js/dataPublic.js（window.DPLootDrop，单一真源禁复制）——登录壳 = 主应用 #page-lootdrop 页签（app.js ensureLootdropMounted 懒挂载，切回 activate 重测；筛选条吸顶 top:56px 让开 topbar，层级规约 筛选条10＞hover卡5 同效），公开壳 = data.html 免登录可分享（body.data-public-body 自动挂载，URL 不动）；anon 直连 PostgREST（字典表 + 公开 RPC get_public_loot_detail，**显式字段白名单——boss_loot/dungeon_loot 加列须 CREATE OR REPLACE 才透出**），全角色只读；毒咒徽标（REQ-110，任务书 #37：venomcurse 列非空时装备卡 meta 行渲染绿色 .dp-tag-venom，sql/26；数据中心录入=预设下拉 无/毒咒 禁自由输入；插件采集已随任务书 #46 插件 1.0.9 落地）；物品图标（REQ-092，任务书 #46：boss_loot/dungeon_loot 加 icon_id（sql/29）+ RPC 白名单透出，卡片右上角 img.dp-item-icon 规则路径 assets/icons/items/{icon_id}.png、lazy+onerror 隐藏、空值不渲染；素材管道 scripts/import-item-icons.js 运营源图入库）；物品等级（REQ-116，S2-装等批次 2026-08-16：boss_loot/dungeon_loot 加 ilvl int 可空（sql/26_s2_ilvl）+ RPC 白名单透出，卡片 meta 行首「物品等级 X」非空才渲染、数据中心双表单录入可空正整数校验；S2 静态表=烈毒按 boss_order 318/321/324/344、潮缚全 318、S2 大米全 311（2026-08-16 运营终裁订正为 292=冒险手册史诗显示档，见行末），S1 不回填；**同档绑定硬性前提=装等显示档与 effect 数值必须同档，注释钉死禁混搭取档**；同批 sql/25_s2_darknight_rollback=至暗之夜世界首领 32 行回滚出库+season 回挂 S1（运营终裁维持世界BOSS不入库，维度行保留）；同批 sql/27_s2_stat_tier_fix=BUG-101 数值同档订正：S2 行 primary/secondary_values←*_tiers->'mythic'（mythic 键在场才动，UPDATE 80/83/181/173=主261/副256），S1 全库无 mythic 键不回填，塞塔里斯神庙 8 件仅 legacy normal 档跳过（sql/29 已销账：7 订正+1 删除））；大米对标手册档（2026-08-16 运营终裁，sql/28_s2_dungeon_journal_ilvl）：S2 大米 dungeon_loot ilvl 311→**292**=冒险手册史诗显示档（勇士 1/6，游戏内两截图实证；UPDATE 207 精确吻合，boss_loot 零触碰，与 sql/27 史诗档数值同档不破），大米卡片数值区下方加灰色小注「装等/数值=冒险手册史诗显示档；实际掉落随钥石层数与宏伟宝库提升」（.dp-item-note，source==='dungeon' 且 ilvl 非空才渲染——团本/巢穴不加注、S1 大米无装等不带注），插件 1.0.27 ilvl_tiers 回流后逐本复核若某本≠292 按本微调；verify-s2-ilvl.js 30/30（含 B6 注行三态/S1 双无断言），版本串 20260816.63；同批 sql/29_s2_temple_remnant_fix=塞塔里斯残桩修复+残项订正（纯数据，schema 不动）：塞塔里斯/化身 7 件订正 292 档实测数值+official_item_id 置换 159xxx→239xxx（BUG-101 跳过清单销账；录库对账匹配键=instance+boss+item_name 故 ID 置换不致重跑插重）、防咬手套（普通难度专属非大米池）+黎明之刃的战刃（外观解锁件）删除、蛰伏盘蛇珍玩 ilvl 344→324（tooltip 截图实证，兑换物以 EJ 自有档轨 285/298/311/324 为准；slot=套装兑换物且 ilvl=344 全库仅此 1 件，盘卷祭坛无兑换物不误伤），S2 大米 205/公示页当前赛季 318/RPC 总量 626，版本串 20260816.64，verify 32/32（含 B2 珍玩 324/B7 删除件绝迹/C6 订正值断言）；S2 批次 sql 编号 24~29 与既有 24_bug062/25_task029/26_req110/27_task042/28_req095/29_req092 撞号属批次自带序号、后缀区分不覆盖；REQ-089 兑换物展开=规则表送审中（docs/REQ-089-兑换物展开规则表-送审.md），308/104/204 基线须运营确认后才准改断言；任务书 #39（REQ-113）：脏标记即时刷新——MasterData 写成功统一口置 window.__dpLootDirty（9 张被消费字典表），activate() 脏则重拉数据段（筛选态/折叠记忆守恒、失败保旧数据+toast+标记留存重试）、不脏零请求只重测溢出；任务书 #43（REQ-098+REQ-110②，=#30 全文+增补）：右栏悬浮筛选面板（≥1400px fixed 四边闭合、独立滚动、--dp-panel-top 壳级顶偏、卡片区让位 margin-right:292px）↔ 折叠顶栏（<1400px 首行恒显+dp43:filterOpen 记忆）两态一套 DOM、命中计数恒显（共 N 件/命中 X 件 · N 项生效）、毒咒筛选组（单选 全部/有毒咒=行 venomcurse 非空，词表 LootTaxonomy.VENOMCURSE_LABEL 与 #37 录入下拉同源）、层级规约 面板 z=10＞hover 卡 z=5

13. **家宅图鉴**（page:decor，任务书 #51 / REQ-137 一期） - **任务书 #59 WP3 起公开壳 decor.html 改为重定向壳（JS location.replace 跳 index.html#/house/decor，query 透传 + meta refresh 兜底；body 摘 data-decor-body 类故渲染层惰性零请求；track.js 保留，PV page='decor' 旧口径不断）**；12.x 家宅装饰全量目录 2062 件（decor_catalog 表，2023 装饰+39 房间/户型），全角色只读零操作；双壳同一数据/渲染层 js/decorData.js（window.DecorCatalog，单一真源禁复制）——登录壳 = #page-decor 页签（app.js ensureDecorMounted 懒挂载；activate 对称占位——只读目录无脏标记链路），公开壳 = decor.html 免登录版（WP2 已交付：只引用 decorDict/decorData/decor-public 三件套+main.css 暗色基底零内嵌，data.html 头部互链一行+版本串同步 20260918.65，server.js 静态白名单放行；WP2 顺手修 WP1 遗留——搜索后计数条不刷新，decorData.js 搜索/清除两处补 refreshCount()）；词表单一真源 js/decorDict.js（货币 25/物品 11 中文名=wago.tools DB2 官方 zhCN+wowhead 中文页、8 主类+31 子类、裸贴图价=货币 3568「封存腐化」非 3448 腐蚀之币——顾问对证点 2026-09-18 查证）；筛选七维（名称/分类主类/来源十类/资料片 tags 组/容量档/摆放环境/可放宠物=官方子分类 53 宠物床 11 件+房间开关）；图标 assets/decor-icons/{icon_file_id}.png + _placeholder.png+onerror 双保险（41 件缺图标）；PostgREST 1000 行上限分页循环拉满后内存索引零请求；分页 60/页+lazy；verify-task51.js 28/28 + verify-task51-wp2.js 39/39，版本串 20260918.65
...
* 插件真机验收工作流（REQ-121，2026-08-14 运营定并验收通过，游戏目录 `C:\Azeroth\World of Warcraft\_retail_\`）：①每次验收前先 diff 游戏目录 `Interface\AddOns\WoWButlerExporter` 与仓库 `addon/WoWButlerExporter`，版本/内容不一致直接喊停，先同步再跑；②跑完直读 `WTF\Account\208457004#1\SavedVariables\WoWButlerExporter.lua`（多账号取最新落盘，免上传）；③本地校验 `node scripts/validate-export.js`（零依赖 Node 直跑，自动定位最新导出文件；输出 V1-V17 文件侧可判项预判清单+件数对账表，判据口径=运营测试步骤卡，聊天侧项标 ⚠️ 不阻塞、❌ 退出码 1；毒咒名单逐件核对=装备八件应中/兑换物五件+flavor 件应空；V1 双跑比对给两个文件参数 `validate-export.js 跑A.lua 跑B.lua`）——插件验收类完工报告必须附该脚本预判结果；④报错栈取证=BugSack 数据文件 `WTF\Account\<账号>\SavedVariables\!BugGrabber.lua`（BugSack.lua 仅为设置）
